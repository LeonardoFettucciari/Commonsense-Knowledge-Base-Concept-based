...
def run_inference(
    ...
    for i, (sample, sample_knowledge) in iterator:
        ...
        for j, prompt in enumerate(prompts):
            ...
            if(prompt.k):
                sample_output['knowledge'] = "\n".join(sample_knowledge[:prompt.k])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset_name", default="allenai/openbookqa", type=str, required=False, help="Dataset name from Hugging Face.")
    parser.add_argument("--kb_path", default="data/ckb_data=obqa_split=test_model=gemini-1.5-flash.jsonl", type=str, required=False, help="Path to the Knowledge Base file.")
    parser.add_argument("--output_dir", default="outputs/inference_1", type=str, required=False, help="Path to store the Knowledge Base.")
    parser.add_argument("--limit_samples", type=int, required=False, help="Maximum number of samples to consider (default: all).")
    parser.add_argument("--top_k_list", type=int, nargs="+", required=False, default=[1, 3, 5, 10, 20], help="List of values of k, statements to inject in prompts with knowledge. (default: 1 3 5 10).")
