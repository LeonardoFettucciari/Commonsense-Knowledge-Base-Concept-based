...
for DATASET_NAME in "${DATASETS[@]}"; do
    for MODEL_NAME in "${MODELS[@]}"; do
        for RETRIEVAL_STRATEGY in "${RETRIEVAL_STRATEGIES[@]}"; do
            echo "Running inference for dataset '$DATASET_NAME' with model '$MODEL_NAME', retrieval strategy '$RETRIEVAL_STRATEGY', input run name '$INPUT_RUN_NAME', output run name '$OUTPUT_RUN_NAME', and prompt '$PROMPT_NAME'"
